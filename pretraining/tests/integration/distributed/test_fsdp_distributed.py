...
class TestFSDPTraining(unittest.TestCase):
    """Test FSDP training with real distributed environment."""
    ...
    def setUpClass(cls):
        ...
        if not dist.is_initialized():
            raise unittest.SkipTest(
                ...
                "torchrun --nproc_per_node=2 test_fsdp_distributed.py"
            )
    ...
    def test_gpt2_fsdp_training(self):
        """Test GPT-2 FSDP training."""
        # Load config
        config_path = "pretraining/configs/examples/debug/gpt2/gpt2_debug_fsdp.yaml"
        trainer_config = loader.load_training_config(config_path, gpt.GPT2Config)

        # Create model
        model = gpt2.GPT2.from_config(trainer_config.llm)

        # Wrap with FSDP using model_wrapper
        ...
        # Verify it's FSDP wrapped
        self.assertIsInstance(dist_model, fsdp.FullyShardedDataParallel)
    ...
    def test_llama3_fsdp_training(self):
        """Test Llama3 FSDP training."""
        # Load config
        config_path = "pretraining/configs/examples/debug/llama3/llama3_debug_fsdp.yaml"
    ...
    def test_deepseek3_fsdp_training(self):
        """Test DeepSeek3 FSDP training with MoE."""
        # Load config
        config_path = "pretraining/configs/examples/debug/deepseek3/deepseek3_debug_fsdp.yaml"
    ...
    def test_fsdp_mixed_precision(self):
        """Test FSDP with mixed precision training."""
        # Load config
        config_path = "pretraining/configs/examples/debug/gpt2/gpt2_debug_fsdp.yaml"
        trainer_config = loader.load_training_config(config_path, gpt.GPT2Config)

        # Override precision to use amp_bf16
        trainer_config.training.precision = precision_configs.PrecisionType.AMP_BF16

        # Create model
        model = gpt2.GPT2.from_config(trainer_config.llm)

        # Wrap with FSDP including mixed precision
        dist_model = model_wrapper.wrap_model(
            model,
            trainer_config.training.execution,
            torch.bfloat16,  # Use bfloat16 for mixed precision
        )
        ...
        # Create minimal data
        dataset = create_dummy_dataset(20, 128)
        sampler = torch.utils.data.DistributedSampler(
            dataset,
            num_replicas=self.world_size,
            rank=self.rank,
        ...
        for batch in train_dataloader:
            ...
            # Check that loss is finite even with mixed precision
            self.assertTrue(
                torch.isfinite(torch.tensor(metrics.get("train/total_loss", float("inf"))))
            )
            break
