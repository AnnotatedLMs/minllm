...
# TODO: Explain weight tying and how it works in GPT-2
# TODO: Explain weight initializing in this module

# TODO: Are these actually optional?
# TODO: SHould they be kwargs? vs explicitly defined?
...
class GPT2LLM(llm.BaseLLM):
    """
    GPT-2 Language Model implementation using pattern abstractions.

    Architecture:
    - Token embeddings + learned position embeddings
    - BiasedLNTransformerBlock (LayerNorm with bias)
    - Multi-head attention (not grouped query)
    - Standard FFN (not MoE)
    - Tied token embeddings for output projection
    """

    def __init__(self, config: llm_configs.GPT2Config):
        super().__init__()
        self.config = config

        assert config.position_embedding_config is not None, "GPT-2 requires position embeddings"
        assert config.transformer_config.rope_config is None, "GPT-2 should not have RoPE"
        ...
        self.position_embeddings = position.LearnedPositionEmbedding(
            max_position_embeddings=config.position_embedding_config.max_position_embeddings,
            embedding_dim=config.position_embedding_config.embedding_dim,
            init_std=config.position_embedding_config.init_std,
        )
        ...
        if config.output_head_config.tie_word_embeddings:
            # Tie weights with token embeddings
            self.lm_head = self.token_embeddings
        else:
            self.lm_head = nn.Linear(
                self.hidden_dim, self.vocab_size, bias=config.output_head_config.lm_head_bias
            )
    ...
    def _create_transformer_block(
        self, config: llm_configs.GPT2Config
        ...
        return transformer.BiasedLNTransformerBlock(
            ...
            bias=config.transformer_config.bias,
