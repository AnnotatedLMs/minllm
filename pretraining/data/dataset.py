...
class PretrainDataset:
    ...
    def _sample_sequence_positions(self, max_pos: int, batch_size: int) -> np.ndarray:
        ...
        For distributed training, we partition the data space so each rank
        sees different data.
        """
        ...
        if world_size > 1:
            # Partition the data space across ranks
            chunk_size = max_pos // world_size
            start_pos = rank * chunk_size
            end_pos = (rank + 1) * chunk_size if rank < world_size - 1 else max_pos
            return np.random.randint(start_pos, end_pos, size=batch_size)
        else:
