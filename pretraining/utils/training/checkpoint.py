...
class Checkpointer:
    ...
    - Preserves best model states during training
    - Manages disk space with checkpoint rotation

    Core Operations:
    - Saves model and optimizer states to disk
    - Loads checkpoints for training resumption
    - Tracks best validation performance

    Training Operations (what checkpointing does):
    1. Serialize model parameters to disk periodically
    2. Save optimizer state (momentum buffers, learning rates)
    3. Record training metadata (iteration, loss, config)
    4. Create atomic saves to prevent corruption
    5. Maintain symlinks to latest/best checkpoints
    6. Rotate old checkpoints to manage disk usage
    7. Load saved states back into model/optimizer for resumption
    ...
    def save_checkpoint(self, checkpoint_data: checkpoint_data_module.CheckpointData) -> None:
        """Save a training checkpoint.

        Args:
            checkpoint_data: CheckpointData object containing all state to save
        """
        # Get iteration from training state
        training_state = checkpoint_data.training_state_dict
        iter_num = training_state.get("iteration", 0)
    ...
    def load_checkpoint(
        self, device: str
    ) -> typing.Optional[checkpoint_data_module.CheckpointData]:
        ...
        # Convert dictionary to CheckpointData
        checkpoint_data = checkpoint_data_module.CheckpointData.from_dict(checkpoint_dict)
