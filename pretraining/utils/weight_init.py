...
class TransformerWeightInitializer:
    ...
    Model Effects:
    - Ensures stable gradient flow through deep networks
    - Prevents vanishing/exploding gradients at initialization
    - Enables training of very deep transformers

    Core Operations:
    - Initializes weights with scaled normal distribution
    - Applies special scaling to residual projections
    - Sets appropriate initial values for norms and biases

    Training Operations (what weight initialization does):
    1. Initialize embeddings/linear weights from N(0, 0.02)
    2. Set LayerNorm weights to 1.0 for identity at start
    3. Set all biases to 0 for unbiased initial predictions
    4. Identify residual projections (attention out, FFN out)
    5. Re-scale residual weights by 1/sqrt(2*n_layers)
    6. This scaling prevents variance explosion across layers
    7. Ensures gradient magnitudes stay stable in deep networks
    """
