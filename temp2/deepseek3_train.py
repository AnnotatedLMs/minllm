...
def get_optimizer(model, config):
    ...
    if optimizer_type == "sgd":
        ...
        return AdamW(optimizer_grouped_params, lr=config["learning_rate"], eps=1e-8)
    else:
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in progress_bar:
            ...
            if isinstance(outputs, tuple):
                ...
                if mtp_outputs is not None:
                    if len(mtp_outputs.shape) == 4:  # [batch, depth, seq_len, vocab_size]
                        ...
                        for k in range(depth):
                            ...
                            shifted_target = torch.full_like(
                                target_ids, config.get("pad_token_id", -100)
                            )
                ...
                main_accuracy = compute_accuracy(
                    logits, target_ids, config.get("pad_token_id", -100)
                )
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0:
                ...
                if not isinstance(scheduler, CosineAnnealingLR) and not isinstance(
            ...
            if (batch_idx + 1) % eval_steps == 0 and (epoch + 1) % eval_epochs == 0:
                val_loss, val_accuracy = validate(
                    model, val_loader, criterion, config.get("pad_token_id", -100), device
                )
                ...
                if val_loss < best_val_loss:
                    ...
                    if patience_counter >= patience:
                        logger.info(
                            f"Early stopping triggered after {patience} validations without improvement"
                        )
        ...
        test_loss, test_accuracy = validate(
            model, test_loader, criterion, config.get("pad_token_id", -100), device
        )
